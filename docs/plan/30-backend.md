# Backend Implementation Plan (`expressync`)

All work is in `/Users/scruffy/Documents/Repos/expressync`. Tech: Fresh +
...
ADMIN_OIDC_ADMIN_GROUP        # e.g. "expressync-admins" — Pocket ID group claim that grants role='admin'
...
```json
{
  "applinks": {
    ...
    "details": [
      {
        "appIDs": ["48H7CLBV8Y.gg.vlad.expresscan"],
        "components": [
          { "/": "/expresscan/register/*" }
        ]
      }
...
4. Backend's `/expresscan/register` page generates a `oneTimeCode` (32-byte
   ...
   60s TTL.
5. Page redirects to `https://manage.polaris.express/expresscan/register/callback?code={oneTimeCode}` — Universal Link.
