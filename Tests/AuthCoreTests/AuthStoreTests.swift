...
//  we don't pollute the real `com.example.expresscharge.ios` keychain. These tests
...
private let keychainIsAvailable: Bool = {
    keychainAvailable(service: "com.example.expresscharge.ios.probe.\(UUID().uuidString)")
}()
...
struct AuthStoreTests {
    ...
    init() {
        let testService = "com.example.expresscharge.ios.tests.\(UUID().uuidString)"
